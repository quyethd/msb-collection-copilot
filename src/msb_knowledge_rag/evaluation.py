...
GOLDEN_QUESTIONS: list[GoldenQuestion] = [
    ...
    GoldenQuestion(
        ...
        expected_evidence=("TASK-011H-V2",),
    ),
    GoldenQuestion(
        question_id="D4",
        group="D",
    ...
    # --- Group C/D: V2 current product + GreenNode live knowledge (TASK-011H-A.1) ---
    GoldenQuestion(
        question_id="C6",
        group="C",
        question="Trang giới thiệu sản phẩm (landing) của hệ thống nằm ở đâu?",
        expected_topics=("PRODUCT", "USER_GUIDE"),
        expected_documents=("16-system-overview-guide",),
        expected_evidence=("đường dẫn gốc", "công khai", "trước khi đăng nhập"),
        forbidden_claims=("nằm trong sidebar", "sau khi đăng nhập"),
    ),
    GoldenQuestion(
        question_id="C7",
        group="C",
        question="Người dùng vào khu vực ứng dụng của hệ thống như thế nào?",
        expected_topics=("PRODUCT", "USER_GUIDE"),
        expected_documents=("02-users-and-use-cases", "16-system-overview-guide"),
        expected_evidence=("Đăng nhập hệ thống", "đăng nhập demo", "đăng nhập"),
        forbidden_claims=("vào khu vực ứng dụng mà không cần đăng nhập",),
    ),
    GoldenQuestion(
        question_id="C8",
        group="C",
        question="Menu bên trong khu vực ứng dụng gồm những trang nào?",
        expected_topics=("PRODUCT", "USER_GUIDE"),
        expected_documents=("17-system-architecture", "11-overview-page-guide"),
        expected_evidence=("Tổng quan", "Danh sách ưu tiên", "Khách hàng", "Tác động dự kiến"),
        forbidden_claims=("Giới thiệu hệ thống",),
    ),
    GoldenQuestion(
        question_id="C9",
        group="C",
        question="GreenNode AI được dùng cho những việc gì trong hệ thống?",
        expected_topics=("GRENNODE", "ARCHITECTURE"),
        expected_documents=("18-greennode-ai-platform", "17-system-architecture"),
        expected_evidence=("AgentBase", "MaaS", "Qwen Flash", "Vector Database"),
        forbidden_claims=("Agent quyết định tuyến",),
    ),
    GoldenQuestion(
        question_id="C10",
        group="C",
        question="Vector Database của GreenNode dùng để làm gì?",
        expected_topics=("GRENNODE", "DATA"),
        expected_documents=("18-greennode-ai-platform", "24-versioning-and-reliability"),
        expected_evidence=("vector kho kiến thức", "chunk", "truy xuất"),
        forbidden_claims=("lưu quyết định khách hàng",),
    ),
    GoldenQuestion(
        question_id="D7",
        group="D",
        question="Mô hình embedding dùng cho kho kiến thức chạy ở đâu?",
        expected_topics=("GRENNODE", "ARCHITECTURE"),
        expected_documents=("18-greennode-ai-platform", "01-product-overview"),
        expected_evidence=("local", "MiniLM", "384"),
        forbidden_claims=("embedding chạy trên MaaS",),
    ),
    GoldenQuestion(
        question_id="D8",
        group="D",
        question="Qwen Flash dùng để làm gì trong hệ thống?",
        expected_topics=("GRENNODE", "EVALUATION"),
        expected_documents=("18-greennode-ai-platform", "21-evaluation-and-trust-evidence"),
        expected_evidence=("tổng hợp", "evidence", "có nguồn"),
        forbidden_claims=("Qwen Flash quyết định khách hàng",),
    ),
    GoldenQuestion(
        question_id="D9",
        group="D",
        question="RAG có tự quyết định khách hàng nên CALL hay CBS không?",
        expected_topics=("TRUST", "ARCHITECTURE"),
        expected_documents=("17-system-architecture", "18-greennode-ai-platform", "20-trust-and-safety"),
        expected_evidence=("Decision Core", "quyết định nghiệp vụ"),
        forbidden_claims=("RAG quyết định tuyến",),
    ),
    GoldenQuestion(
        question_id="D10",
        group="D",
        question="Kho kiến thức hiện tại có đang chạy trên GreenNode vDB không?",
        expected_topics=("GRENNODE", "DATA"),
        expected_documents=(
            "18-greennode-ai-platform",
            "24-versioning-and-reliability",
            "21-evaluation-and-trust-evidence",
            "17-system-architecture",
        ),
        expected_evidence=("TASK-011H-V2", "OpenSearch", "live"),
        forbidden_claims=("chưa chạy trên vDB",),
    ),
    GoldenQuestion(
        question_id="D11",
        group="D",
        question="RAG đã được tích hợp vào Trợ lý production chưa?",
        expected_topics=("GRENNODE", "ROADMAP"),
        expected_documents=("21-evaluation-and-trust-evidence", "25-roadmap-and-next-steps", "18-greennode-ai-platform"),
        expected_evidence=("COPILOT_INTEGRATION", "NO", "chưa tích hợp"),
        forbidden_claims=("đã tích hợp sản xuất", "đang chạy production"),
    ),
    # --- Boundary group ---
    GoldenQuestion(
        question_id="BND1",
